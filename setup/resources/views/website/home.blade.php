                ...
                <div class="hero-content mx-auto max-w-[780px] text-center">
                    ...
                    <ul class="mb-10 flex flex-wrap items-center justify-center">
                        <li><a href="{{$post->settings['hero_button_url'] ?? ''}}"
                                class="inline-flex items-center justify-center rounded-lg bg-white py-4 px-6 text-center text-base font-medium text-dark transition duration-300 ease-in-out hover:text-primary hover:shadow-lg sm:px-10">{{$post->settings['hero_button_text']
                                ?? ''}}</a></li>
                    </ul>
                </div>
            </div>
            ...
            <div class="w-full px-4">
                <div class="relative z-10 mx-auto max-w-[845px]">
                    <div class="mt-16">
                        ...
                        <img src="{{config('app.url')}}/uploads/{{$post->settings['hero_image']}}"
                            alt="{{$post->title ?? ''}}" class="mx-auto max-w-full rounded-t-xl rounded-tr-xl" />
                        @endif
                    </div>
        ...
        <div class="-mx-4 flex flex-wrap">
            <div class="w-full px-4">
                <div class="mb-12 max-w-[620px] lg:mb-20">
                    <span
                        class="mb-2 block text-lg font-semibold text-primary">{{$post->settings['feature_section_name']
                        ?? ''}}</span>
                            ...
                            <dl class="mt-3 max-w-xl space-y-2 text-base leading-7 text-gray-600 lg:max-w-none">
                                ...
                                <div class="relative pl-9">
                                    ...
                                    <dt class="inline font-semibold text-gray-900"><svg
                                            ...
                                            fill="currentColor" class="absolute top-1 left-1 h-5 w-5 text-indigo-600"
                                            viewBox="0 0 16 16">
                                            <path
            ...
            <div class="w-full px-4 lg:w-5/12 xl:w-4/12">
                <div
                    ...
                    <form method="post" action="{{$base_url}}/signup">
                        ...
                        <div class="mb-6">
                            <label for="password_confirmation" class="block text-xs text-dark">{{__('Confirm
                                Password')}}</label>
                            <input type="password" id="password_confirmation" placeholder="{{__('Retype Password')}}"
                        ...
                        <div class="mb-6">
                            <button type="submit"
                                class="inline-flex items-center justify-center rounded bg-primary py-4 px-6 text-base font-medium text-white transition duration-300 ease-in-out hover:bg-dark">{{__('Get
                                Started')}}</button>
                        </div>

                        <div class="mb-2">
                            <small class="text-gray-300">{{__('By clicking the button, you agree to our Terms of Service
                                and have read and acknowledge our Privacy Statement.')}}</small>
                        </div>

                        <div class="mb-3">
                            <small>
                                <a class="text-blue-400" href="{{$base_url}}/privacy-policy">{{__('Privacy
                                    Policy')}}</a> | <a class="text-blue-400"
                                    href="{{$base_url}}/terms-of-service">{{__('Terms of Service')}}</a>
                            </small>
                        </div>

                        <div class="mb-0">
                            {{__('Already have an account?')}} <a class="text-primary"
                                href="{{$base_url}}/office/login">{{__('Sign in')}}</a>
                        </div>
                    ...
                    <div class="items-center justify-between overflow-hidden border lg:flex">
                        ...
                        <div class="text-center">
                            <div class="relative z-10 inline-block">
                                @if(!empty($post->settings['hero_image']))
                                <img src="{{config('app.url')}}/uploads/{{$post->settings['hero_image']}}"
                                    alt="{{$post->title ?? ''}}" class="mx-auto lg:ml-auto" />
                                @endif
                            </div>
                        </div>
                    </div>
    ...
    <div class="container">
        ...
        <div class="flex flex-wrap items-center justify-center">
            ...
            <div class="w-full md:w-1/2 lg:w-1/3">
                <div
                    class="relative z-10 mb-10 overflow-hidden rounded-xl {{$subscription_plan->is_featured ? 'bg-primary bg-gradient-to-b from-primary to-[#179BEE]' : 'border border-primary border-opacity-20'}} bg-white py-10 px-8 text-center shadow-pricing sm:p-12 lg:py-10 lg:px-6 xl:p-12">

                    @if($subscription_plan->is_featured)
                    <span
                        class="mb-5 inline-block rounded-full border border-white bg-white py-2 px-6 text-base font-semibold uppercase text-primary">{{$subscription_plan->featured_text
                        ?? __('Popular')}}</span>
                    @endif


                    <span
                        class="mb-2 block text-base font-medium uppercase {{$subscription_plan->is_featured ? 'text-white' : 'text-dark'}}">{{$subscription_plan->name
                        ?? ''}}</span>

                    @if($subscription_plan->price_monthly && $subscription_plan->price_monthly > 0)
                    <h2
                        class="mb-9 text-[28px] font-semibold {{$subscription_plan->is_featured ? 'text-white' : 'text-primary'}}">
                        {{formatCurrency($subscription_plan->price_monthly ?? '',getWorkspaceCurrency($super_settings),
                        true)}}/{{__('month')}}
                    </h2>
                    ...
                    <h2
                        class="mb-9 text-[28px] font-semibold {{$subscription_plan->is_featured ? 'text-white' : 'text-primary'}}">
                        {{formatCurrency($subscription_plan->price_yearly ?? '',getWorkspaceCurrency($super_settings),
                        true)}}/{{__('year')}}
                    </h2>
                    ...
                    <div class="mb-10">
                        ...
                        <p
                            class="mb-1 text-base font-medium leading-loose {{$subscription_plan->is_featured ? 'text-white' : 'text-body-color'}}">
                            {{$feature}}</p>
                        @endforeach
                    </div>
                    @endif
...
<section id="faq" class="relative z-20 overflow-hidden bg-[#f3f4ff] pt-20 pb-12 lg:pt-[120px] lg:pb-[90px]">
    <div class="container">
        <div class="-mx-4 flex flex-wrap">
            <div class="w-full px-4">
                <div class="mx-auto mb-[60px] max-w-[620px] text-center lg:mb-20"><span
                        class="mb-2 block text-lg font-semibold text-primary">
                ...
                <div class="ud-single-testimonial mb-12 bg-white p-8 shadow-testimonial">
                    ...
                    <div class="ud-testimonial-info flex items-center">
                        <div class="ud-testimonial-meta">
                            <h4 class="text-sm font-semibold">{{$post->settings['testimonials_author'][$key] ?? ''}}
                            </h4>
                            <p class="text-xs text-[#969696]">{{$post->settings['testimonials_author_title'][$key] ??
                                ''}}</p>
                        </div>
                    </div>
                </div>
